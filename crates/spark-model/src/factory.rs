...
pub fn loader_for_config(config: &ModelConfig) -> Result<Box<dyn ModelWeightLoader>> {
    ...
    match normalized.as_str() {
        ...
        "glm5_next" => Ok(Box::new(Glm5NextWeightLoader)),
        _ => bail!(
            "Unsupported model type: '{}' (normalized: '{}'). \
            Supported: qwen3_next, qwen3_5_moe, qwen3_5, qwen3_6_moe, holo3_1_moe, qwen3_vl_moe, nemotron_h, nemotron_h_puzzle, gemma4, mistral, minimax_m2, step3p7, laguna, deepseek_v4, glm5_next, m2m_100",
            config.model_type,
            normalized,
        ),
    }
